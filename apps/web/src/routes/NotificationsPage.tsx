import { Bell, Filter, CheckCircle2, ChevronRight, Monitor } from 'lucide-react'
      ...
      <div className="kg-card mt-4 overflow-hidden">
        {filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-slate-900">{t('notifications.empty')}</h3>
            <p className="mt-2 text-sm text-slate-500 max-w-sm">{t('notifications.emptyBody')}</p>
          </div>
        ) : (
            ...
            {filteredItems.map(item => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`mt-1 text-sm ${!item.read ? 'text-slate-700' : 'text-slate-500'}`}>
                    {item.body}
                  </p>
                </div>
  ...
  return (
    <button
      className={`rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-1 ${
        active ? 'bg-teal-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
      }`}
      onClick={onClick}
      type="button"
    >
