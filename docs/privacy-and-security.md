...
The seated camera flow requires the user to confirm that they intend to use the camera while seated. This confirmation is transient React state on the camera-setup page: it is discarded when the page is left or reloaded and is not sent to an API, persisted, or logged. The camera checks technical landmark visibility only and does not automatically determine whether the user is seated or whether their posture is correct.
