...
The educational clinical-flow prototype adds a deny-by-default content layer between consent and the existing movement-demo journey. Go owns mock screening routing and production-review eligibility. React keeps MediaPipe extraction in-browser, and Python receives only transient pose status plus bounded visibility scores for technical camera feedback. See `docs/educational-clinical-flow.md`.
